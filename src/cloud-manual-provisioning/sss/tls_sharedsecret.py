...
class TLS:
    ...
    def __init__(self, session_obj):
        ...
        for i in range(self.server_rand_val_len):  # pylint: disable=unused-variable
    ...
    def do_calculate_shared_secret(self, key_id, prv_key_id, psk_key_id):  # pylint: disable=not-an-iterable
        """
        Perform decryption
        ...
        status = apis.Se05x_API_TLSCalculatePreMasterSecret(ctypes.byref(self._session.s_ctx),
                                                            prv_key_id, psk_key_id, key_id, shared_secret_ctype,
    ...
    def do_tls_perform_prf(self, key_id):
        ...
        status = apis.Se05x_API_TLSGenerateRandom(ctypes.byref(self._session.s_ctx), client_rand_val_ctype,
                                                  ctypes.byref(ctypes.c_size_t(self.client_rand_val_len)))
