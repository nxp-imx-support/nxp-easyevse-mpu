...
class DeriveKey:  # pylint: disable=too-many-instance-attributes
    ...
    def _symmetric_context_init(self, key_id):
        ...
        if status != apis.kStatus_SSS_Success:
            return status
        if self.cipher_type==apis.kSSS_CipherType_NONE :
            return apis.kStatus_SSS_Fail
        if self.object_type==apis.kSSS_KeyPart_NONE:
            return apis.kStatus_SSS_Fail
    ...
    def symmetric_derive_kd(self, key_id, salt, info, req_key_len, derive_key_id=None,
                            salt_id=None):
        ...
        if salt is not None:
            ...
            status, object_type, cipher_type = self._ctx_key.get_handle(salt_id)  # pylint: disable=unused-variable
