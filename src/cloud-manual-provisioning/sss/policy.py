...
class Policy:
        ...
        for i in range(1, self._policy_count + 1):
            ...
            if policy_key.type == apis.KPolicy_Sym_Key:
                if bool(pol_obj_params['desfire_change_key_%s' % i]):
                    ...
                    policy_desfire_changekey.policy.desfire_auth_id.desfire_authId = pol_obj_params[
                        'desfire_auth_id_%s' % i]
    ...
    def create_policy_buffer(self, policy_in=None):  # pylint: disable=no-self-use
        """
