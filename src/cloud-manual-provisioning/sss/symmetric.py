...
class Symmetric:
    ...
    def mac_multi_step(self, key_obj, src_data, src_data_len, dest_data=None, dest_data_len=None):
        ...
        if dest_data is None:
            ...
            status = apis.sss_mac_finish(ctypes.byref(self.mac_ctx), ctypes.byref(dest_ctype),
                                         ctypes.byref(dest_len_ctype))
            if status != apis.kStatus_SSS_Success:
    ...
    def aead_one_shot(self, key_obj, src_data, tag, tag_len):
        ...
        status = apis.sss_aead_one_go(ctypes.byref(self.aead_ctx), ctypes.byref(src_data_ctype),
                                      ctypes.byref(dest_data_ctype),
                                      dest_data_len, nonce_ctype, self.nonce_len, aad_ctype, self.aad_len,
                                      ctypes.byref(tag_ctype), ctypes.byref(tag_len_ctype))
