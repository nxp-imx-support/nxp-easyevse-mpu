...
int seGetClientKey(sss_key_store_t *keyStore, int keyId, U8 *clientKey, size_t *clientKeyLen)
{
    sss_object_t keyObject;
    size_t keyLenBits = *clientKeyLen * 8;
    sss_status_t status;

    status = sss_key_object_init(&keyObject, keyStore);
    if (status != kStatus_SSS_Success)
    {
        printf("Failed to initialize keyObject: status code = 0x%04x.\n", status);
        return SE_ERR_SE_GP_NO_CERT;
    }
    ...
    status = sss_key_store_get_key(keyStore, &keyObject, clientKey, clientKeyLen, &keyLenBits);
    if (status != kStatus_SSS_Success)
    {
        printf("Fetching client key fails with status code: 0x%04x.\n", status);
        return SE_ERR_SE_GP_NO_CERT;
    }

    printf("Size of key retrieved: %zu byte\n", *clientKeyLen);
